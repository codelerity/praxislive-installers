...
public class Exec {
    ...
    void build() throws Exception {
        ...
        String version = config.getProperty("praxislive.version", "");
        if (version.isBlank()) {
            throw new IllegalStateException("PraxisLIVE version not set");
        }
    ...
    void validate(boolean downloads) throws Exception {
        String version = config.getProperty("praxislive.version", "");
        if (version.isBlank()) {
            throw new IllegalArgumentException("PraxisLIVE version not set");
        }
    ...
    List<String> buildNBPackageCommandLine(Path nbpackage, Path praxislive,
            Path pkgConfig, Path dist, Path jdk, String version) {
        List<String> cmd = new ArrayList<>();
        cmd.add(nbpackage.toString());
        cmd.add("--verbose");
        cmd.add("--input");
        cmd.add(praxislive.toString());
        cmd.add("--config");
